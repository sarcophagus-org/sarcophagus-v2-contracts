...
describe("Contract: SignatoryFacet", () => {
  describe("registerSignatory", () => {
    it("registers a signatory", async () => {
      ...
      const registeredSignatory = await viewStateFacet.getSignatoryProfile(
        signatory.signatoryAddress
      );
      expect(registeredSignatory.exists).to.be.true;
    });

    it("fails to register a signatory when it is already registered", async () => {
    ...
    it("initializes the cursedBond to 0", async () => {
      ...
      expect(registeredSignatory.cursedBond).to.equal(BigNumber.from("0"));
    });
    ...
    it("initializes the profile config values correctly", async () => {
      ...
      expect(registeredSignatory.minimumDiggingFee).to.equal(BigNumber.from(minDiggingFee));
      expect(registeredSignatory.maximumRewrapInterval).to.equal(BigNumber.from(maxRewrapInterval));
      expect(registeredSignatory.freeBond).to.equal(BigNumber.from(freeBond));
      expect(registeredSignatory.peerId).to.equal(peerId);
    });
    ...
    it("adds the signatory address to the signatoryProfileAddresses array", async () => {
      ...
      const registeredSignatoryAddress = await viewStateFacet.getSignatoryProfileAddressAtIndex(0);
      expect(registeredSignatoryAddress).to.equal(signatory.signatoryAddress);
    });

    it("deposits free bond to the vault contract when registering with a positive free bond value", async () => {
      const { signatories, signatoryFacet, heritageToken } = await signatoriesFixture(1);
      const signatory = signatories[0];
      ...
      const vaultContractBalanceBefore = await heritageToken.balanceOf(signatoryFacet.address);
      ...
      const vaultContractBalanceAfter = await heritageToken.balanceOf(signatoryFacet.address);
  ...
  describe("updateSignatory", () => {
    it("updates a signatory values successfully", async () => {
      const { signatories, signatoryFacet, viewStateFacet } = await signatoriesFixture(1);
      const signatory = signatories[0];
      ...
      const signatoryFreeBondBeforeUpdate = await viewStateFacet.getFreeBond(signatory.signatoryAddress);
      ...
      expect(registeredSignatory.minimumDiggingFee).to.equal(BigNumber.from(minDiggingFee));
      expect(registeredSignatory.maximumRewrapInterval).to.equal(BigNumber.from(maxRewrapInterval));
      expect(registeredSignatory.freeBond.sub(signatoryFreeBondBeforeUpdate)).to.equal(
        BigNumber.from(freeBond)
      );
      expect(registeredSignatory.peerId).to.equal(peerId);
    });
    ...
    it("reverts when a signatory is not registered", async () => {
      const { signatories, signatoryFacet } = await signatoriesFixture(1);
      const signatory = signatories[0];
  ...
  describe("depositFreeBond()", () => {
    ...
    context("with a registered signatory", () => {
      it("deposits free bond to the contract", async () => {
        ...
        const { signatories, signatoryFacet, viewStateFacet, heritageToken } =
        ...
        const signatoryVaultBalanceBefore = await heritageToken.balanceOf(
          signatory.signatoryAddress
        );
        ...
        const freeBond = await viewStateFacet.getFreeBond(signatory.signatoryAddress);
        expect(freeBond.toString()).to.equal(amountToDeposit);

        const signatoryVaultBalanceAfter = await heritageToken.balanceOf(
          signatory.signatoryAddress
        );
        ...
        const contractSarcBalance = await heritageToken.balanceOf(signatoryFacet.address);
      ...
      it("emits event DepositFreeBond()", async () => {
        ...
        await expect(tx)
          ...
          .withArgs(signatory.signatoryAddress, 100);
      });
    });
    ...
    context("with a registered signatory with positive free bond deposit", () => {
      context("Successful withdrawals", () => {
        it("withdraws free bond from the contract", async () => {
          ...
          const signatoryBalanceBefore = await heritageToken.balanceOf(contextSignatory.signatoryAddress);
          ...
          const freeBond = await viewStateFacet.getFreeBond(contextSignatory.signatoryAddress);
          expect(freeBond.toString()).to.equal("0");

          const signatoryBalanceAfter = await heritageToken.balanceOf(contextSignatory.signatoryAddress);

          expect(signatoryBalanceAfter.toString()).to.equal(signatoryBalanceBefore.toString());
        ...
        it("should emit an event when the free bond is withdrawn", async () => {
          ...
          await expect(tx)
            ...
            .withArgs(contextSignatory.signatoryAddress, 100);
        });
        ...
        it("should emit a transfer event when the vault token is transfered", async () => {
          ...
          const tx = await signatoryFacet
            .connect(contextSignatory.signer)
            .withdrawFreeBond(BigNumber.from(100));
          await expect(tx).emit(heritageToken, "Transfer");
        });
      });
  ...
  describe("withdrawReward()", () => {
    it("withdraws all the signatories rewards", async () => {
      ...
      const signatoryDiggingFee = BigNumber.from("1000000000000");

      // Setup signatory + unwrap so rewards are received
      const {
        signatories,
        signatoryFacet,
        vaultId,
        ...
        { shares, threshold, signatoryMinDiggingFee: signatoryDiggingFee },
        "Test Vault"
      );
      ...
      const currentRewards = await viewStateFacet.getRewards(contextSignatory.signatoryAddress);
      expect(currentRewards).to.equal(signatoryDiggingFee);

      const signatoryVaultBalanceBefore = await heritageToken.balanceOf(contextSignatory.signatoryAddress);
      ...
      const rewardsAfterWithdrawal = await viewStateFacet.getRewards(
        contextSignatory.signatoryAddress
      );
      expect(rewardsAfterWithdrawal).to.equal(0);

      // expect signatories vault token balance to increase by rewards amount
      expect(await heritageToken.balanceOf(contextSignatory.signatoryAddress)).to.equal(
        signatoryVaultBalanceBefore.add(signatoryDiggingFee)
      );
    });
  });
  ...
  describe("unwrapVault()", () => {
    ...
    context("Successful unwrap", () => {
      it("should store the unencrypted shard on the contract", async () => {
        ...
        const signatory = await viewStateFacet.getVaultSignatory(
          vaultId,
          signatories[0].signatoryAddress
        );
      ...
      it("should transfer the digging fee to the signatory's reward pool without transferring tokens", async () => {
        ...
        const vaultBalanceBefore = await heritageToken.balanceOf(signatories[0].signatoryAddress);
        const signatoryRewardsBefore = await viewStateFacet.getRewards(signatories[0].signatoryAddress);
        ...
        const vaultBalanceAfter = await heritageToken.balanceOf(signatories[0].signatoryAddress);
        const signatoryRewardsAfter = await viewStateFacet.getRewards(signatories[0].signatoryAddress);
        ...
        expect(signatoryRewardsAfter.toString()).to.equal(signatoryRewardsBefore.add(totalFees).toString());
      });
    ...
    context("Failed unwrap", () => {
      ...
      it("should revert if the sender is not a signatory on this vault", async () => {
  ...
  describe("finalizeTransfer()", () => {
    ...
    context("Successful transfer", () => {
      it("should update the list of signatories on a vault", async () => {
        ...
        expect(signatoryAddresses).to.contain(newSignatory.signatoryAddress);
    ...
    context("Failed transfer", () => {
      ...
      it("should revert if the provided signature is not from a signatory on the vault", async () => {
