 ...
 */
export const failingFinalizeFixture = deployments.createFixture(
  async ({ deployments, getNamedAccounts, getUnnamedAccounts, ethers }) => {
    ...
    const arweaveArchaeologistSignature = await sign(
    ...
    const arweaveArchBalance = await sarcoToken.balanceOf(arweaveArchaeologist.account);
