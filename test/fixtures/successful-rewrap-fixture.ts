 ...
 */
export const successfulRewrapFixture = deployments.createFixture(
  async ({ deployments, getNamedAccounts, getUnnamedAccounts, ethers }) => {
    ...
    const sarcoBalances = await Promise.all(
      archaeologists.map(async archaeologist => await sarcoToken.balanceOf(archaeologist.account))
    );

    const oldResurrectionWindow = (await viewStateFacet.getSarcophagus(identifier))
      .resurrectionWindow;
